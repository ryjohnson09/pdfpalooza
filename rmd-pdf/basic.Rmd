...
country_closest_match <- country_names[which.min(stringdist::stringdist(country_focus, country_names, method = "jaccard", q = 3))]
...
country_data |> 
  ...
  # ) |> 
  tab_header(
    title = paste(country_focus, "Overview"),
...
In @ref(fig:fig-life-expectancy), we can view the life expectancy of `r country_focus` compared to every other country in the gapminder dataset.

```{r fig-life-exectancy, fig.cap="Life expectancy plot. Focus country is shown in orange. Every other country is shown in grey."}

ggplot(gapminder_filt, aes(x = year, y = lifeExp, group = country)) +
      geom_line(aes(color = ifelse(country == country_focus, country_focus, "Other")),
                linewidth = ifelse(gapminder_filt$country == country_focus, 1.5, 0.7), 
                alpha = ifelse(gapminder_filt$country == country_focus, 1, 0.3)) +
      scale_color_manual(values = setNames(c("gray", "#EE6331"), c("Other", country_focus))) +
      labs(title = paste(country_focus, "Life Expectancy"),
           x = "Year",
...
In @ref(fig:fig-population), we can view the population of `r country_focus` compared to every other country in the gapminder dataset.

```{r fig-population, fig.cap="Population plot. Focus country is shown in orange. Every other country is shown in grey."}
...
ggplot(gapminder_filt, aes(x = year, y = pop, group = country)) +
      ...
      scale_color_manual(values = setNames(c("gray", "#EE6331"), c("Other", country_focus))) +
      scale_y_log10() +
      labs(title = paste(country_focus, "Population"),
           x = "Year",
           y = "Population",
           color = "Country") +
      theme_minimal() +
...
In @ref(fig:fig-gdp), we can view the population of `r country_focus` compared to every other country in the gapminder dataset.

```{r fig-gdp, fig.cap="GDP Per Cap plot. Focus country is shown in orange. Every other country is shown in grey. GDP per capita measures the value of everything produced in a country during a year, divided by the number of people. The unit is constant dollars adjusted for inflation in 2017’s prices."}
...
ggplot(gapminder_filt, aes(x = year, y = gdpPercap, group = country)) +
      ...
      scale_color_manual(values = setNames(c("gray", "#EE6331"), c("Other", country_focus))) +
      scale_y_log10() +
      labs(title = paste(country_focus, "GDP per Cap"),
           x = "Year",
           y = "GDP Per Cap",
           color = "Country") +
      theme_minimal() +
